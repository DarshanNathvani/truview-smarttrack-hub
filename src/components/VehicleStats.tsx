  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        {/* Speed and Motion with Firebase IDs */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            <div id="speed" className="text-3xl font-bold text-trutrack-blue mb-2">{vehicleData.speed}</div>
            <div className="text-sm text-gray-600">km/h</div>
            <div className="text-xs text-gray-500 mt-1">Current Speed</div>
          </div>
          <div className="text-center">
            <div id="ignitionStatus" className="text-lg font-semibold text-gray-900 mb-2">{vehicleData.ignition}</div>
            <div className="w-4 h-4 bg-trutrack-green rounded-full mx-auto mb-1" />
            <div className="text-xs text-gray-500">Ignition</div>
          </div>
          <div className="text-center">
            <div id="motionStatus" className="text-lg font-semibold text-gray-900 mb-2">{vehicleData.motion}</div>
        ...
        {/* Battery and Signal with Firebase IDs */}
        <div className="space-y-4">
          <div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-gray-700">Battery Level</span>
              <span className="text-sm text-gray-600"><span id="batteryLevel">{vehicleData.battery}</span>%</span>
            </div>
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm text-gray-600"><span id="signalStrength">{vehicleData.signal}</span>%</span>
            </div>
            <Progress value={vehicleData.signal} className="h-2" />
          </div>
        </div>

        {/* Last Update with Firebase ID */}
        <div className="flex justify-between items-center pt-4 border-t border-gray-100">
          <span className="text-sm text-gray-600">Last Update</span>
          <span id="lastUpdateTime" className="text-sm font-medium text-gray-900">{vehicleData.lastUpdate}</span>
        </div>
      </CardContent>
    </Card>
  );
